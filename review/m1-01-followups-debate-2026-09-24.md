...
| B2 | 新增 `http.public_url` 作为主地址，备用地址放 settings | 修改：已有 `ui.portal.api_base_url`（DEP-04）；备用地址放 settings 等于允许后台把全部客户端引到任意域名 | 主地址取 `ui.portal.api_base_url`，否则取用户中心公开地址；备用地址放部署配置 `client.api_endpoints`，不进 settings 与管理接口；每项语义同 `api_base_url`，可带路径前缀（spec/30 API-10） |
