...
适用范围：全部仓库。选型的决策记录见 ADR 0009、0010、0014、0016。状态为“提议”的选型可在 M0 结束前修改。
...
| OpenAPI 代码生成 | oapi-codegen（strict server 模式） | 从 `panel-spec` 生成类型与接口，手写只实现业务 |
| 数据库驱动 | pgx v5 | PostgreSQL 专用，性能与功能最好 |
| SQL | sqlc | SQL 写在 `.sql` 文件中生成类型安全的 Go 代码，Claude 容易审查 |
| 迁移 | goose | SQL 文件迁移，支持在 Go 中嵌入 |
| Valkey 客户端 | valkey-go | Valkey 官方客户端，支持自动流水线与客户端缓存 |
...
| 配置 | 环境变量 + 单一 YAML 文件，启动时校验 | 简单、可在容器中覆盖 |
...
| 本地存储 | 单个目录下的文件：凭据（0600）、配置快照、WAL；不引入嵌入式数据库 |
...
| 用途 | 选型 |
|---|---|
| 前置 Web 服务器 | 可选。控制面二进制已内置前端；需要自动 HTTPS 时在前面放 Caddy |
| 本地与单机部署 | Docker Compose |
| 多实例部署 | Helm Chart（M3） |
| CI | GitHub Actions |
| 发布 | GoReleaser、cosign 签名、Syft 生成 SBOM |
| 容器镜像仓库 | GHCR |
