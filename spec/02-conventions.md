...
- **CONV-11** 分页为游标分页：请求 `?limit=50&cursor=...`，响应 `{ "items": [...], "next_cursor": "..." }`。
  ...
  - 条目数可能超过 200 的列表必须分页。以下列表可以不分页，但必须在 OpenAPI 描述中写明上限：数量受设备上限约束的设备列表（只返回未吊销的设备），以及由运营者维护、只返回生效中内容的公告与文章列表。
...
**例外**：`/v1/oauth/*` 端点按 RFC 6749 §5.2 与 RFC 8628 返回 `application/json` 的 `{error, error_description}`，不使用 problem+json。这是 CONV-16 唯一的例外。
...
- **CONV-19** 敏感值用应用层 AEAD 加密，列名以 `_enc` 结尾。敏感值包括：节点密钥、TOTP 密钥、支付私钥、DNS 服务商凭据、代理凭据、导出令牌明文（CONV-20）。
