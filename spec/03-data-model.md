...
| 至多一个免费套餐 | 部分唯一索引 `plans_one_free_uq ON plans ((true)) WHERE kind = 'free'`（spec/11 BIL-15） |
| 价格行周期与套餐类型匹配 | 插入触发器 `plan_prices_plan_guard`：免费套餐不设价格行；`recurring` 只用四种周期且 `period_days` 为空；`one_time` 只用 `one_time`（spec/11 BIL-01） |
...
| `plans` 列 | `version`（`bigint NOT NULL DEFAULT 1`） | 套餐 ETag 的来源（spec/31 CON-05、CONV-28）。应用在修改套餐、新建或停售其价格行、添加或移除线路组关联时，在同一事务中加 1（backlog M1-04） |
| `location_groups` 列 | `version`（`bigint NOT NULL DEFAULT 1`） | 线路组 ETag 的来源。应用在修改线路组、增减节点成员时，在同一事务中加 1（`host_count` 因此随版本变化）；节点状态变化不加 1；套餐关联变化只影响派生字段 `plan_ids`，不加 1（CON-05） |
| `plans` 索引 | `plans_one_free_uq`、`plans_sort_id`（`sort, id`） | 前者见 3.3；后者用于管理接口 `GET /v1/plans` 的游标分页（按 `sort`、`id` 升序，CONV-11）。该表已有数据，两个索引以单独的迁移 `CREATE INDEX CONCURRENTLY` 建立并标记 `-- +goose NO TRANSACTION`（CONV-21）；版本列与触发器放在另一个普通迁移中 |
