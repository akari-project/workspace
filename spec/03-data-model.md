...
| 表 | `plan_rollouts` | 权益快照批量修改的执行记录（spec/11 BIL-02、BIL-15）。列：`id`（UUIDv7）；`plan_id`（引用 `plans(id)`，`ON DELETE RESTRICT`）；`kind`（`apply` 应用到现有用户、`free_grant` 授予免费权益、`free_end` 结束免费权益）；`fields`（`text[]`，`apply` 时为 `bytes_per_cycle`、`device_limit`、`speed_limit_mbps`、`reset_policy` 的非空子集，其他 kind 为空数组，CHECK）；`target_values`（`jsonb`，创建时冻结的目标值，键与 `fields` 相同，其他 kind 为 `{}`）；`status`（`queued`、`running`、`completed`、`failed`）；`affected_account_count`、`processed_account_count`（`int`，≥ 0）；`cursor`（`uuid`，可空，已处理到的权益 ID 或账号 ID）；`reason_id`（引用 `reason_texts(id)`，`NOT NULL`）；`actor_id`（`uuid NOT NULL`，创建记录的管理员；免费套餐记录为修改设置的管理员）；`attempts`（`int`，连续失败次数）；`last_error`（`text`，可空，不含个人信息，CONV-29）；`completed_at`（可空）；`created_at`、`updated_at`（`touch_updated_at`）。部分唯一索引 `plan_rollouts_one_active`（`plan_id`）`WHERE status IN ('queued','running')`；索引（`plan_id`, `created_at`, `id`）用于管理接口分页。执行者由 `job_fencing` 租约保证唯一 |
| `usage_cycles` 列 | `bytes_limit`（`bigint NOT NULL DEFAULT 0`，≥ 0）、`bytes_carried`（`bigint NOT NULL DEFAULT 0`，≥ 0） | 前者为该周期的实际额度（被本段起点或终点截断的周期按 BIL-16 折算；0 表示不限），开周期时由应用写入，本段终点移动时重算当前周期；后者为“只按时间”折算预置的已用量（BIL-12），计入本周期已用量。该表在 M1 没有生产数据，默认值只为满足 `NOT NULL` |
| `settings` 键 | `proration_mode` | 折算方式（spec/11 BIL-12），JSON 字符串 `"value"`（按价值，默认）或 `"time_only"`（只按时间）；管理接口字段同名。缺键或值异常按 `"value"` 处理，记 warn 日志只记键名。报价与开通校验（ORD-05）读取同一有效值 |
