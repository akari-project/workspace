...
- **ORD-06** 临界续费：续费订单在权益有效期内创建、并在订单有效期内完成支付的，即使支付时权益刚好到期，仍按续费处理并保留锁定价格。开通一个从支付时刻开始的新段，流量周期也从支付时刻开始。
  - 价格在报价与下单时确定（续费按 BIL-07）。开通时 ORD-05 第 2 步的重算仍按续费场景、以报价时的锁定价格进行，不因权益已到期改按新购重算。这是领域层规则，由开通逻辑保证，不依赖接口。
...
- **ORD-10** 退款成功后，写入余额流水（原路退回时为审计记录）与 `refund` 权益事件：
  ...
  - 本段实付减去退款额；
  - 分笔（spec/11 BIL-20）：`refund` 事件的 `diff.tranches` 给出调整后的完整列表。退款额从最晚的一笔开始扣减，扣到 0 后再扣前一笔；当前时刻之后的区间端点 t 映射为 当前时刻 + floor((t − 当前时刻) × (1 − 退款额 ÷ 退款前的剩余价值))，与剩余时长的缩短一致，缩为空的笔删除；
  - 剩余时长降为 0 时权益 `ended`；
  - 优惠券次数不归还。
...
| `downgrade_surplus` | 正 | 立即降级时剩余价值超出所选价格行金额的部分（spec/11 BIL-23） |
