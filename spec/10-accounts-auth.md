...
- **AUTH-19** 以下为敏感操作，需要以下三项：
  ...
  敏感操作包括：手动标记支付、余额调整、退款、切换节点内核、修改支付配置、应用到现有用户、从套餐移除线路组、吊销节点密钥、管理员与角色变更、重置用户密码、导出审计日志或用户数据（`/v1/audit-logs/exports`、`/v1/accounts/{id}/data-exports`）。执行者还必须拥有该操作对应的权限（AUTH-17）。
- **AUTH-25** 账号暂停（管理员操作）：
