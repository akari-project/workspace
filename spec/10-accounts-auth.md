...
- **AUTH-03** 邮箱验证码为 6 位数字，15 分钟有效，最多尝试 5 次。重新发送每个账号每分钟 1 次、每天 10 次；发送新码后旧码作废。未验证邮箱的账号可以登录，但下单返回 `email_unverified`。
- **AUTH-04** 找回密码：
...
- **AUTH-08** 浏览器中的刷新令牌放在 HttpOnly、Secure、SameSite=Strict 的 Cookie 中；自研客户端存入系统安全存储。
- **AUTH-09** 登录限流：
...
- **AUTH-10** 登录时同时注册设备，记录平台、型号、应用版本、设备公钥：
  - `platform='web'`（浏览器中的用户中心）的登录只注册 web 设备，不生成代理凭据，也不计入设备上限。
  - 其他平台的客户端登录时可以带上已有的 `device_id`，并用该设备私钥对服务端下发的 nonce 签名；验证通过则复用原设备记录与名额，否则注册新设备。
...
- **AUTH-17** 权限以字符串表示，格式为 `资源.动作`，资源名使用管理接口的路径名（如 `hosts`，而不是 `nodes`），`资源.*` 表示该资源的全部动作，`*` 表示全部权限。管理接口 OpenAPI 的每个操作以 `x-permission` 声明一个权限，取值只能来自下表，由 CI 校验；中间件统一校验权限。
...
- **AUTH-19** 以下为敏感操作，需要以下三项：
  ...
  敏感操作包括：手动标记支付、余额调整、退款、切换节点内核、修改支付配置、应用到现有用户、从套餐移除线路组、吊销节点密钥、管理员与角色变更、重置用户密码、导出审计日志或用户数据。执行者还必须拥有该操作对应的权限（AUTH-17）。
- **AUTH-25** 账号暂停（管理员操作）：
