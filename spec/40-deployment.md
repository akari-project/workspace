...
- **DEP-10** 备份与恢复：
  ...
  - 加密主密钥与控制面的签名私钥（CONV-30：`PANEL_MASTER_KEY`、`PANEL_TOKEN_KEY`、`PANEL_CONFIG_KEY`）不随数据库备份，必须由运营者离线备份，与数据库备份分开存放。部署文档写明这一要求，以及主密钥丢失的后果：全部 `_enc` 值无法解密，需要重新接入节点、重置 TOTP、重新录入支付私钥、轮换全部代理凭据。
  - Agent 与客户端安装包的发布签名密钥由项目发布流程离线保管（CONV-30、DEP-09），不由运营者备份。
