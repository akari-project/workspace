...
5. 在 `node-agent/go.mod` 更新 `replace` 指向新 tag，运行 `make test && make conformance`。
6. 一致性测试覆盖全部协议 × 两个内核，失败项逐个分析是上游行为变化还是补丁问题，把上游行为变化记录到 `node-agent/internal/kernel/CLAUDE.md` 的“已知差异”。
7. 规模较大时，使用 `workflows/core-sync.md` 中的 Dynamic Workflow 并行执行第 5–6 步。
